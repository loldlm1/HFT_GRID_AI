...
- `Grid_Risk_Trend_Mode` optionally enforces Alligator exits on the timeframe you select via `Grid_Risk_Timeframe_Source` (base or trend). Pair it with `Grid_Risk_Alligator_Reference` to decide whether the jaws or teeth line acts as the breaker: `GRID_RM_TREND_OFF` disables it, `GRID_RM_TREND_JAWS_BE` closes the grid (only at ≥ break-even) when the most recent level entry sits beyond that line, `GRID_RM_TREND_JAWS_SL` turns that comparison into a hard stop, and `GRID_RM_TREND_JAWS_SAR` flips the grid to the opposite direction (reusing the latest level’s lot size) whenever the breach happens.
