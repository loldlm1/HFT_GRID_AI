...
- **Trailing strategy**: `Grid_Trailing_Strategy_Mode` picks between price-offset trailing, ATR trailing rails (shift=1), or Alligator lips MA (shift=1). Indicator-based trailing adds the configured `Grid_Trailing_TP_Percent` offset to the indicator value and clamps with `MathMax`/`MathMin` so the trailing anchor never retreats below (bullish) / above (bearish) the indicator. `Grid_Trailing_Execution_Mode` defaults to price-triggered trailing; `TRAILING_EXECUTION_AGGRESIVE` waits until the indicator clears the TP reference—and switches to the would-be stop/next-level price whenever `Grid_Level_Stop_Limit` is in play—before enabling trailing so reversals always lock profit.
