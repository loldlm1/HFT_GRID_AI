...
- `Grid_Risk_Trend_Mode` optionally enforces Alligator jaws exits: `GRID_RM_TREND_OFF` disables it, `GRID_RM_TREND_JAWS_BE` closes the grid (only at ≥ break-even) when the most recent level entry sits beyond the jaws line, `GRID_RM_TREND_JAWS_SL` turns that comparison into a hard stop, and `GRID_RM_TREND_JAWS_SAR` flips the grid to the opposite direction (reusing the latest level’s lot size) whenever the breach happens.
