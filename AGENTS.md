...
`Global_Indicator_Percent` (default `-1`) overrides every context’s Bollinger percent input whenever you need to sweep a single threshold during parameter searches. `ENTRY_OFF` fully disables signals for that context (it still updates the cascade trend state), while `ENTRY_ON_TREND` lets the context open grids purely off its Alligator trend as long as the associated `Strategy_*_Trend_Mode` is not `TREND_OFF`.
...
- `ATR_RANGE` and `KELTNER_RANGE` pull spacing from the respective volatility channel (`ATR_SL_Factor` or `Keltner_Channel`). `Grid_Channel_Factor` feeds both indicators, and the base distance is always measured against the smoothed support/resistance lines (ATR SMA bands or the Keltner upper/lower rail).
