...
- `Grid_Risk_Trend_Mode` optionally enforces Alligator Lips-based exits: `GRID_RM_TREND_OFF` disables it, `GRID_RM_TREND_LIPS_BE` closes the grid (only at ≥ break-even) when the most recent level entry sits beyond the Lips line, and `GRID_RM_TREND_LIPS_SL` treats that entry vs Lips comparison as a hard stop.
