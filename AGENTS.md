...
| --- | --- | --- |
| Base | Executes on `Strategy_Timeframe`. `Strategy_Base_Entry_Evaluation` chooses the Bollinger/Keltner confirmation (window, mean, both, or `ENTRY_EVAL_ON_TREND`), while `Strategy_Base_Trend_Mode` toggles the Alligator branch (`TREND_OFF`, jaws, or teeth). Slope toggles (`Base_BPercent_Slope_Filter`, `Base_Stochastic_Slope_Filter`, `Base_Alligator_Slope_Filter`) use the same >=/<= guards as the higher contexts, and the trio `Strategy_Channel_Indicator_Type` + `Strategy_Global_Entry_Mode` + `Strategy_Global_Entry_Evaluation_Mode` controls whether every context hydrates Bollinger Percent or Keltner Percent data, whether breakouts/MA trends/reversions map to the 0/50/100 presets, and whether a single evaluation overrides per-context settings. Structure filters, Fibonacci retests, and `Base_Fresh_Structure_Time` gate swings by snapshotting `first_structure_time` by default or `second_structure_time` whenever `Base_Second_Structure_Filter` is active so grids only open once per structure; these structure/fresh toggles only impact the context’s own signals and no longer block the cascade. | `services/trading_management/ea_inputs.mqh` |
...
`Strategy_Channel_Indicator_Type` chooses whether every context hydrates Bollinger Percent or the new Keltner Percent dataset. `Strategy_Global_Entry_Mode` maps the breakout / MA-trend / reversion presets to the canonical 0 / 50 / 100 thresholds, and `Strategy_Global_Entry_Evaluation_Mode` can override each context’s evaluation whenever you want to sweep a single confirmation style. Set a context’s evaluation to `ENTRY_EVAL_OFF` to keep its cascade trend state without letting it spawn grids, or `ENTRY_EVAL_ON_TREND` to let the associated Alligator branch authorize entries on its own.
...
### 2.2 Grid Framework
- **Spacing**: `ATR_RANGE` vs `POINTS_RANGE`. ATR mode clamps the recalculated base distance so it can’t be smaller than the last realised spacing (prevents hyper-aggressive levels when ATR contracts). When `Strategy_Global_Entry_Mode` is `ENTRY_MODE_BREAKOUT` or `ENTRY_MODE_REVERSION`, the base distance is measured from price back to the channel midline (MA) because the signal fires at the extremes; `ENTRY_MODE_MA_TREND` keeps using the support/resistance rails.
- `ATR_RANGE` and `KELTNER_RANGE` pull spacing from the respective volatility channel (`ATR_SL_Factor` or `Keltner_Channel`). `Grid_Channel_Factor` feeds both indicators, and the channel guard for pending stops still references the smoothed support/resistance lines (ATR SMA bands or the Keltner upper/lower rail).
