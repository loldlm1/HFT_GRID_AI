...
| XBoost stats files | XBoost stores CSV files in Common storage with versioned names like `pandora_xboost_v5_<strategy>_<symbol>_<period>_stats.csv`, `_samples.csv`, and `_broker_trades.csv`. Sample ids are loaded once and skipped when a repeated tester run sees the same day/node/outcome again. | Run the same training range twice and confirm sample counts do not duplicate. Delete or archive these files to reset training data for a preset. |
| XBoost session mask | When `session_mask.csv` exists directly in MT5 `Common\Files`, XBoost loads it automatically and applies its `train_allowed` / `trade_allowed` gates. If the file is absent, mask mode is disabled. | For clean Dukascopy tests, place the validator mask at `Common\Files\session_mask.csv`; delete or rename that file to disable mask mode for normal broker/baseline runs. |
...
| XBoost panel lines | The panel/tester comment shows `XBOOST <mode> v<schema> root=<side> day=<date> d=<depth> broker=<n>/<max>` plus up to three `XB#` candidate rows with id, status, samples, V5 score, posterior R, median R, profit factor, broker node evidence, and reason. | Use the panel to anticipate the next branches that may open real positions; it is display-only and does not affect trading decisions. |
...
- Place a trusted `session_mask.csv` directly in MT5 `Common\Files` only when the dataset provides `date,status,train_allowed,trade_allowed`; remove or rename that file to disable mask mode.
...
- With `Enable_File_Logs = true`, confirm `PANDORA_XBOOST_DRYRUN` includes `score`, `v5`, `ar`, `cr`, `sr`, `shr`, `sfrag`, `sbr`, calendar fields (`w120_days`, `w60_days`), and sample fields (`s120`, `s60`, `age`).
- For XBoost `BLOCK` candidates, check whether the reason is `SAMPLES_<have>_<need>`, `V5_RECENT`, `V5_SCORE`, `STALE_SAMPLE`, `SESSION_MASK`, or `BROKER_DEGRADATION`.
...
| XBoost inference replay | Run the same range with `Pandora_XBoost_Mode = PANDORA_XBOOST_INFERENCE` using stats from the training pass. | Candidate rows show `READY`, `WAIT`, `WATCH`, or `BLOCK`; broker selected logs appear only for READY candidates. `PANDORA_XBOOST_DRYRUN` should show `score`, `v5`, `ar`, `cr`, `sr`, `s120`, `s60`, and `age`. | READY selected broker entries use comments like `pandora_xb_pos_1`. | This confirms wiring and gates only; it is not a valid edge backtest because the same data trained the stats. |
...
| XBoost max-depth stop | Set `Pandora_XBoost_Max_Depth = 1`, `2`, `3`, `4`, or `5` and force enough closes to reach the limit. | No branch advances beyond the configured max depth. | Broker count never exceeds the configured max depth. | Derived local stats can continue only up to max depth, and real broker selections stop at the depth limit. |
| XBoost session mask audit | Place `session_mask.csv` directly in MT5 `Common\Files`; run around known clean, excluded, and warmup dates. | `PANDORA_XBOOST_MASK_LOAD` appears once. Excluded or missing dates log `PANDORA_XBOOST_SAMPLE_SKIP reason=SESSION_MASK`; `trade_allowed=false` logs `PANDORA_XBOOST_BROKER_SKIP reason=SESSION_MASK`. | Local visual progression can still advance for audit, but no blocked-date broker entry should appear. | Mask mode is for curated Strategy Tester data only; delete or rename `Common\Files\session_mask.csv` for normal broker/live operation. |
