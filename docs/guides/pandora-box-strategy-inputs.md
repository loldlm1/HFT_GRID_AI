...
- Pandora can retry broker execution for transient open failures only. The retry budget affects broker execution capture, not the deterministic local entry time/price or local SL/TP.
...
| Broker retry entries | Retry pending means the local entry already exists and the EA is only trying to attach real broker execution inside the configured retry budget/window. | Confirm retry success does not move the local entry price/time, and retry give-up still closes locally later. |
...
|---|---|---|---|---|
| High spread blocks broker send | `Pandora_Box_Max_Entries = 1`, valid box, breakout tick, and `Max_Spread` below observed spread. | `PANDORA_ENTRY_OPEN`, then active/last error reason such as `PANDORA_BROKER_SPREAD_BLOCK` or `GUARDRAIL_BLOCK`; broker retry may execute only after spread returns inside limits and retry guards pass. | Pending/final local marker like `... (Posicion local - ERR_Spread)` unless broker retry executes, then `... (Posicion ejecutada)`. | `open=1/1`; next breakout on the same day does not create a second local entry until the first local entry closes. Broker history exists only if retry succeeds. |
| Transient broker send failure | Configure or reproduce `TRADE_RETCODE_ERROR`, price changed/off, requote, timeout, connection, or too-many-requests while local entry remains active. | First failure records diagnostics and sets broker retry pending; later eligible ticks retry until success, attempts exhausted, window expired, or drift exceeded. | Executed retry: `... (Posicion ejecutada)`. Give-up: local marker with the last reject reason. | Local entry price/time and local SL/TP remain unchanged; broker fill may differ and should be measured as execution drift. |
