...
| XBoost session mask audit | Configure `Pandora_XBoost_Session_Mask_File` with a Common Files CSV containing `date,status,train_allowed,trade_allowed`; run around known clean, excluded, and warmup dates. | `PANDORA_XBOOST_MASK_LOAD` appears once. Excluded or missing dates log `PANDORA_XBOOST_SAMPLE_SKIP reason=SESSION_MASK`; `trade_allowed=false` logs `PANDORA_XBOOST_BROKER_SKIP reason=SESSION_MASK`. | Local visual progression can still advance for audit, but no blocked-date broker entry should appear. | Mask mode is for curated Strategy Tester data only; leave the input empty for normal broker/live operation. |
