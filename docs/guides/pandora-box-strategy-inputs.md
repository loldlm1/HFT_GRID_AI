...
- For invalid stops, Pandora first checks exact broker SL/TP, then broker-safe wide SL/TP, then a market request with no initial broker SL/TP when that no-stop request is otherwise valid. If all valid market candidates fail, the entry is treated as `local_rejected` and remains active until local SL/TP, BE, or trailing logic closes it.
- Invalid volume is repaired once by refreshing symbol constraints and renormalizing volume; if it still fails, it is final. Margin, market closed/disabled, close-only, and account/symbol restrictions are not simulated as normal trades.
