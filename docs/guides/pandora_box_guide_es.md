...
- Si el spread esta fuera de rango en la ruptura, Pandora espera antes de crear la entrada local activa. Si stops invalidos, volumen, margen u otra regla operable del broker bloquea el envio despues de tener un anchor broker-realistic, Pandora mantiene la entrada local viva como `local_rejected` hasta que el SL/TP/BE/trailing local la cierre. Market closed/disabled no se simula como trade normal.
...
- Que el SL/TP broker se valide como proteccion extra; el SL/TP local exacto se basa en el fill real o anchor simulado activo aunque los stops broker esten temporalmente mas amplios.
