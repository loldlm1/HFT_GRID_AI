...
## Product
- Name: `HFT Grid AI - Base EA`
- Type: `Core product`
- SKU: `base_ea`

## Short Copy Block
`HFT Grid AI Base gives you the full core engine: risk controls, strategy context, and execution settings. Start with safe defaults and add premium modules only when needed.`

## Medium Copy Block
`The Base EA is the foundation package for HFT Grid AI. It includes the complete core workflow: account-level protections, market context controls, and position sizing settings. This means users can run a solid version of the system without buying addons on day one.`
