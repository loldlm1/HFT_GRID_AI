...
def _groups(rows: list[dict[str, Any]]) -> list[_IdentityGroup]:
    grouped: dict[tuple[str, str, str], list[int]] = {}
    for index, row in enumerate(rows):
        grouped.setdefault(_identity_key(row), []).append(index)
    groups = [
        _IdentityGroup(
            key=key,
...
def build_time_splits(
    ...
    metadata = {
        "policy": "chronological_pivot_window_holdout_plus_walk_forward",
        "grouping_policy": "pivot_window_identity",
        "group_columns": ["run_id", "symbol", "window_id"],
