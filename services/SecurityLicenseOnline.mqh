//+------------------------------------------------------------------+
#ifndef _SERVICES_SECURITY_LICENSE_ONLINE_MQH_
#define _SERVICES_SECURITY_LICENSE_ONLINE_MQH_
...
{
  if(error_code == "invalid_source")
    return true;
  if(error_code == "invalid_key")
    return true;
  if(error_code == "addons_required")
    return true;
  if(error_code == "trial_disabled")
    return true;
  return false;
}
...
void LicenseOnline_OnTimer()
{
  ...
  if((now - last_validation_time) < license_refresh_seconds)
    return;
...
#endif // _SERVICES_SECURITY_LICENSE_ONLINE_MQH_
