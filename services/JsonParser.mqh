    ...
    public:
        ...
        string toString() const {
            ...
            for (int i = 0; i < arraySize; i++) {
                ...
                string escaped_key = JSON::_escapeString(item.key);
                result += "\""+ escaped_key +"\": " + item.toString() + (i < arraySize -1 ? "," : "");
            }
...
private:
    ...
    static string _escapeString(const string &value) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            else if (ch == 8) result += "\\b";
            else if (ch == 12) result += "\\f";
            else if (ch == '\n') result += "\\n";
    ...
    class JsonValueItem {
        public:
            ...
            string toString() const {
                if (this.valueType == JSONBoolType) return (this.booleanValue ? "true" : "false");
                if (this.valueType == JSONNumberType) return JSON::_numberToString(this.doubleValue);
                ...
                if (this.valueType == JSONStringType) {
                    string copy = JSON::_escapeString(this.stringValue);
                    return "\"" + copy + "\"";
                }
