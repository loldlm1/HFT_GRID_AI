...
bool DailyResults_IsAuthError(const string error_code)
{
  ...
  if(error_code == "expired")
    return true;
  if(error_code == "license_not_found")
    return true;
  if(error_code == "user_not_found")
    return true;
  if(error_code == "ea_not_found")
    return true;
  return false;
}
...
void DailyResults_ScheduleRetry(const datetime now_server,
                                const int fixed_delay_seconds = 0)
{
...
void DailyResults_MarkReportedDay(const datetime day_start_utc,
