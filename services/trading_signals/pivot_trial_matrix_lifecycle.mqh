                                    ...
                                    const MqlTick &origin_tick)
{
  ...
  bool active_state_complete = true;
  ...
  return rows_complete && active_state_complete && signal.matrix_declared;
}
                                 ...
                                 PivotTrialOutcome &outcome_out)
{
  ...
  outcome_out.chain_terminal_reason = tp_touched
                                      ? PIVOT_TRIAL_CHAIN_TP_REACHED
                                      : (trial.identity.sl_policy ==
                                         PIVOT_TRIAL_SL_STRUCTURAL
                                         ? PIVOT_TRIAL_CHAIN_STRUCTURAL_SL
                                         : PIVOT_TRIAL_CHAIN_INELIGIBLE);
  return true;
}
