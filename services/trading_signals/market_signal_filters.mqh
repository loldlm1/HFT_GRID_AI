...
bool EvaluateDeterministicBaseSetup(const int strategy_id,
                                    ...
                                    double &ma_prev_out)
{
  ...
  if(direction == BULLISH && extremum.is_peak)
    return false;
  if(direction == BEARISH && !extremum.is_peak)
    return false;
  ...
  int extremum_shift = iBarShift(_Symbol,
  ...
  if(direction == BULLISH)
    return (extremum.extremum_low < ma_now_out && ma_now_out > ma_prev_out);

  if(direction == BEARISH)
    return (extremum.extremum_high > ma_now_out && ma_now_out < ma_prev_out);
                                            ...
                                            double &ma_prev_out)
{
  ...
  ENUM_TIMEFRAMES macro_tf = DeterministicStrategyMacroTimeframe(strategy_id);
