                                ...
                                const datetime terminal_time)
{
  if(!PivotV13Enabled() ||
    ...
    return;
  if(!PivotV13RecordWindow(window, terminal_time, "EXPIRED"))
    return;
...
void FinalizeActivePivotWindowsForExport()
{
  ...
  if(!PivotV13RecordWindow(g_pivot_fractal_window,
                           TimeCurrent(),
                           "RUN_FINISHED"))
    return;
  string window_id = PivotV13WindowId(_Symbol,
...
{
  signal_out.Reset();
  signal_out.window_id = PivotV13WindowId(_Symbol,
                                          candidate.window.timeframe,
                                          candidate.window.active_bar_open);
  signal_out.origin_id = PivotV13OriginId(_Symbol,
                                          candidate.window.timeframe,
                                          candidate.window.active_bar_open,
                                          candidate.level_id);
  signal_out.broker_signal_id =
    PivotV13BrokerSignalId(signal_out.origin_id);
