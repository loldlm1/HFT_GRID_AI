...
bool PivotV12UpdateOrigin(const PivotSignal &signal)
{
  ...
  g_pivot_v12_pending_origins[index].h1_lanes_declared = signal.h1_lanes_declared;
  return true;
}
                          ...
                          const string terminal_status)
{
  ...
  PivotV12AppendColumn(row, PivotV12BoolToken(pending.h1_lanes_declared));
...
bool PivotV12RecordVirtualTrial(const PivotTrialEntry &trial)
{
  if(!PivotV12Ready())
    return false;
  bool matrix_trial = trial.identity.role == PIVOT_TRIAL_ROLE_H1;
  bool parity_trial =
  ...
  PivotV12AppendColumn(row,
                       matrix_trial
                       ? PivotTrialEntryPolicyLabel(trial.identity.entry_policy)
                       : PIVOT_V12_NULL);
  PivotV12AppendColumn(row,
                       matrix_trial
...
bool PivotV12RecordVirtualOutcome(const PivotTrialOutcome &outcome)
{
  ...
  bool matrix_outcome =
    outcome.identity.role == PIVOT_TRIAL_ROLE_H1;
  bool parity_outcome =
  ...
  PivotV12AppendColumn(
    row,
    matrix_outcome
    ? PivotTrialEntryPolicyLabel(outcome.identity.entry_policy)
    : PIVOT_V12_NULL);
  PivotV12AppendColumn(row,
                       matrix_outcome
...
bool PivotV12MarkOriginMatrixDeclared(const string origin_id)
{
  ...
  if(index < 0)
    return false;
  g_pivot_v12_pending_origins[index].h1_lanes_declared = true;
  return true;
}
