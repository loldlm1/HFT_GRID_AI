...
- **Distance Modes**
  ...
  - `Grid_ATR_Range_Mode` selects which ATR buffers feed the ladder when `ATR_RANGE` is active: classic support/resistance windows, the trailing rails, both (pick whichever is safer), the raw ATR “root” bands (`BufferMAUpper`/`BufferMALower`), or the new ATR SMA references (simple moving averages of each band). The SMA branch reuses the peaks (upper mean) for bearish grids and the bottoms (lower mean) for bullish ones so spacing can track a smoothed version of the wick-weighted envelopes.
  - When `ATR_RANGE` is enabled, `Grid_Points_Range_Setup` now doubles as a hard floor against the ATR SMA reference: if the distance between the pending stop and the relevant ATR SMA line falls short of that floor, the signal is blocked (or the pending stop is cancelled if it was already seeded) so ultra-thin ladders never reach the market. Once a grid fills level 0 the guard is skipped, but the original spacing is preserved for the rest of the sequence.
