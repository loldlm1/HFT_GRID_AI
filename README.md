...
3. **Trend / Macro / Session Context Inputs**
   - `Strategy_Trend_Entry_Mode` / `Strategy_Trend_Trend_Mode` mirror the base menu and enforce their confirmations independently on the selected timeframe. Trend, macro, and session contexts now produce their own signals while simultaneously feeding the cascade gate that lower contexts must satisfy before triggering.
