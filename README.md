...
- **Lot Modes**
  - `GRID_LOT_SIZE`, `GRID_LOT_PERCENTAGE_BASED`, `GRID_LOT_CURRENCY_BASED`.
  ...
  - Legacy enum values from older `.set` files are normalized to `GRID_LOT_SIZE` at runtime and logged once.
- **Trend Risk Guard**
...
| **Grid Risk** | `Grid_Lot_Type` (`GRID_LOT_SIZE` / `GRID_LOT_PERCENTAGE_BASED` / `GRID_LOT_CURRENCY_BASED`), `Grid_Lot_Strategy_Size`, `Grid_Lot_Multiplier` (martingale/ladder), `Grid_Level_Position_Start` (skip live broker entries until this grid level so earlier steps stay virtual), `Grid_Level_Stop_Limit` (max depth before force-close), `Daily_Signal_Limit` + mode (caps total or losing grids per day). Trend guard now sits in its own group with `Grid_Risk_Trend_Mode` (off / BE / SL / SAR / HEDGE), `Grid_Risk_Alligator_Reference` (pick jaws or teeth), `Grid_Risk_Timeframe_Source` (base/trend/macro/session), `Grid_Risk_Trend_Timeframe` to force a dedicated TF, plus hedge inputs `Grid_Risk_Trend_Hedge_Points`, `Grid_Risk_Trend_Hedge_SL`, and `Grid_Risk_Trend_Hedge_Level_Cover`. |
