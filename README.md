...
- Structure filters: `Base_First/Second/Third/Fourth_Structure_Filter`, `Base_Support_Filter`, `Base_Resistance_Filter`, `Base_Min_Extern_Structures_Broken`. These filters (and the fresh-structure toggles) only control whether the context can open its own grid—they do not block downstream contexts in the cascade. When multiple structure filters are enabled, the highest-order one controls the “fresh” timestamp used to avoid duplicate grids. Flip on `*_First_Structure_Close_Percent` when you prefer to sample the live close percent for that swing instead of the completed extremum.
...
Per-context runtime state lives in `market_signal_state.mqh`: each context slot tracks its last evaluated bar time, trend readiness/pass per direction, and the fresh-structure timestamp so `DetectStrategySignals()` can stay stateless inside the loop without scattered globals.
...
4. **Admission Flow**
   ...
   2. `StrategyContextEvaluateTrend()` updates the trend state for the active context, while `StrategyCascadeAllowsSignal()` ensures lower timeframes only fire when every upstream context with an enabled trend mode is currently green. Upstream trend snapshots are refreshed once per base-context bar so the gate stays aligned with fresh base decisions without re-processing higher layers on every tick.
...
- **Lot Modes**
  - `PANDORA_LOT_SIZE`, `PANDORA_LOT_PERCENTAGE_BASED`, `PANDORA_LOT_CURRENCY_BASED`.
  - Percentage and currency modes convert the selected account metric (balance percentage or explicit currency budget) into lots using the live entry→TP span.
  - Invalid enum values fall back to `PANDORA_LOT_SIZE` and are logged once.
- **Pandora Box Window**
  - `Pandora_Box_Time_Range` accepts `HH:MM-HH:MM` same-day windows (`start < end`) and overnight windows (`start > end`, for example `23:00-00:10`). Overnight boxes belong to the day they close and use the last known closed D1 candle day for the start side, so broker-specific weekend history is respected. Identical start/end values are invalid because they do not define a usable box range.
  - `Pandora_Box_Entry_Type` defaults to `ENTRY_WICK_TYPE`, preserving the current tick/current-price breakout. `ENTRY_BODY_TYPE` uses `Pandora_Box_Entry_Body_Timeframe` (default `PERIOD_M5`) and triggers only once per direction when the last closed candle closes inclusively beyond the offset breakout level.
  - `Pandora_Box_Use_Session_Filter` remains an entry-attempt gate only; it does not decide whether the box construction window itself is valid.
- **Trend Risk Guard**
- `Grid_Risk_Trend_Mode` compares the most recent grid level entry price against the Alligator reference you pick via `Grid_Risk_Alligator_Reference` (jaws or teeth) and on the timeframe selected through `Grid_Risk_Timeframe_Source` (strategy/trend/macro/session) or overridden with `Grid_Risk_Trend_Timeframe` when you want a dedicated TF. `GRID_RM_TREND_OFF` disables it, `GRID_RM_TREND_BE` closes the sequence (only when aggregate floating P/L is ≥ 0) once the latest filled level sits on the wrong side of the reference line, `GRID_RM_TREND_SL` treats that line as a hard stop, `GRID_RM_TREND_SAR` flips the grid into the opposite direction (using the latest level’s lot size) whenever the breach occurs so the sequence “s.ar”s between bullish and bearish states until price trends back, and `GRID_RM_TREND_HEDGE` pairs every filled level with an opposite-side hedge at the same lot size. The hedge uses `Grid_Risk_Trend_Hedge_Points` (or the grid’s base distance when left at 0) for its SL buffer, optionally removes that SL once the next level sits further than that buffer, and auto-closes the whole grid + hedge once at least `Grid_Risk_Trend_Hedge_Level_Cover` open-position levels are in profit.
...
6. **License & Daily API v1**
   - `POST /api/v1/licenses/verify` runs at startup and refreshes on timer (`license_refresh_seconds`). Non-2xx or `ok=false` responses fail validation, and refresh failures remove the EA.
...
| **Grid Strategy** | `Grid_Base_Strategy_Type`, `Grid_Points_Range_Setup`, `Grid_Channel_Evaluation_Factor`, `Grid_Channel_Volatility_Factor` (fallback to `Grid_Channel_Factor`), exponential multiplier, TP/stop percentages, volatility clamp is automatic. |
...
| **Pandora Risk** | `Pandora_Box_Time_Range` supports same-day (`start < end`) and overnight (`start > end`) boxes; overnight boxes close on the active day and start from the last known closed D1 candle day. `Pandora_Box_Entry_Type` selects `ENTRY_WICK_TYPE` tick/current-price breakout or `ENTRY_BODY_TYPE` closed-candle breakout on `Pandora_Box_Entry_Body_Timeframe` (default `PERIOD_M5`). `Pandora_Lot_Type` (`PANDORA_LOT_SIZE` / `PANDORA_LOT_PERCENTAGE_BASED` / `PANDORA_LOT_CURRENCY_BASED`), `Pandora_Lot_Strategy_Size`, `Pandora_Points_SL`, `Pandora_Points_TP`, `Pandora_Points_Value_Mode` (`POINTS` vs `%` of pure box range), and `Pandora_Risk_Trailing_Mode` (`OFF` / `STEP_TP`). In `STEP_TP`, hard TP auto-close is disabled and the SL advances by the initial SL range (BE at TP1, then TP1 at TP2, etc.) with unlimited milestones. `Pandora_Box_Max_Entries` now gates entries on opens (`0` unlimited), while `PANDORA DONE` is emitted only after those budgeted entries close; `PANDORA WAIT_CLOSE` marks the transition state. `Pandora_Box_Entry_Count_Mode` remains a close-outcome analytics counter (SL/TP/BE), including BE in SL/TP modes. Re-entries arm only after `close_1` returns inside the box; body mode uses the body timeframe for that rearm. Global controls (`Grid_Lot_Multiplier`, `Grid_Level_Position_Start`, `Grid_Level_Stop_Limit`, `Daily_Signal_Limit`) still govern lifecycle depth/budget. Trend guard remains under `Grid_Risk_Trend_*` inputs. |
| **Developer Debug** | Logging toggles, chart options, `Enable_Trend_Filter_Sanity_Stop`, `Debug_Stop_On_Negative_Euity`. |

Pandora entries are deterministic local entries first. `Pandora_Box_Max_Entries` counts the local operation even when the broker blocks or rejects the send for spread, stops, volume, margin, or market state; those entries remain active locally until exact local SL/TP/BE logic closes them. With `Pandora_Box_Set_Broker_SLTP`, invalid broker stops are handled through exact SL/TP, broker-safe wide SL/TP, then no initial broker SL/TP when the no-stop market request passes `OrderCheck`; broker protection later tightens toward the exact local targets. Chart markers distinguish `Posicion ejecutada` from local rejected labels such as `Posicion local - ERR_Spread`, and broker history should not be treated as the source of local Pandora statistics.
