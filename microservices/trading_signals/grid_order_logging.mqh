                                 ...
                                 const string result_comment)
{
  ...
  string protection_policy = PandoraBrokerProtectionPolicyLabel(signal_params,
                                                               request.sl,
                                                               request.tp);
  double effective_stop_points = PandoraBrokerEffectiveProtectionDistancePoints();

  string message = StringFormat("dir=%s|symbol=%s|level=%d|status=%s|vol=%.2f|req_price=%.5f|sl=%.5f|tp=%.5f|sl_policy=%s|local_sl=%.5f|local_tp=%.5f|protect_sl=%.5f|protect_tp=%.5f|entry_ref=%.5f|entry=%.5f|bid=%.5f|ask=%.5f|spread=%.1f|max_spread=%.1f|magic=%I64u|comment=%s|attempt=%d/%d|retry_mode=tick|ret=%I64u|err=%d|ret_desc=%s|result_comment=%s|check=%s|check_ret=%u|check_err=%d|check_margin=%.2f|check_free=%.2f|check_level=%.2f|check_comment=%s|trade_mode=%d|exec_mode=%d|filling=%d|order_mode=%d|vol_min=%.2f|vol_max=%.2f|vol_step=%.2f|point=%.5f|tick=%.5f|freeze=%.1f|stops=%.1f|eff_stop=%.1f",
                                 direction,
                                 request.symbol,
                                 order_state.level_index,
                                 EnumToString(order_state.status),
